...
class YellowTrafficLight(TrafficObject):

    def set_car_state(self, car_state):
        logging.debug('yellow light: Be cautious')
        car_state['speed'] = 0.1
...
class Pedestrian(TrafficObject):
    ...
    def set_car_state(self, car_state):
        logging.debug('pedestrian: stop car')

        car_state['speed'] = 0

class curve(TrafficObject):

    def set_car_state(self, car_state):
        logging.debug('curvy road: Road ahead curves')

        car_state['speed'] = 0.3


class yeild(TrafficObject):

    def set_car_state(self, car_state):
        logging.debug('yield: Slow down')
